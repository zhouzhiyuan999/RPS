...
<form action="formAction!edit" method="post">
    <input type="hidden" name="did" value="<s:property value="data._id"/>">
<table class="table table-bordered table-hover definewidth m10">
    ...
    <%--<s:bean name="com.rps.comparator.SeqComparator" id="seqComparator">--%>
        <%--<s:sort source="form.items" comparator="seqComparator">--%>
            <s:iterator value="form.items" id="item" status="st">
                <tr>
                    <td class="tableleft"><s:property value="#item.name"/><s:if test="#item.isneed==true">*</s:if></td>
                    <td><input type="<s:property value="#item.datatype"/>" name="<s:property value="#item.label"/>" value="<s:property value="data.data.get(#st.index)"/>"/></td>
                </tr>
            </s:iterator>
        <%--</s:sort>--%>
    <%--</s:bean>--%>
...
<script>
    $(function () {       
		$('#backid').click(function(){
				window.location.href="formAction!check?fid=<s:property value="form._id"/>";
		 });
    });
</script>